    ...
    peerings: [
      {
        ...
        remotePeeringName: 'hub-to-server${parIndex}'
...
module modVirtualMachine 'br/public:avm/res/compute/virtual-machine:0.20.0' = {
  name: 'deploy-server${parIndex}-vm-${parLocation}'
  params: {
    ...
    name: 'vm-server${parIndex}-${parLocation}'
    nicConfigurations: [
      {
        ipConfigurations: [
          {
    ...
    extensionCustomScriptConfig: {
      ...
      settings: {
        fileUris: [
          'https://raw.githubusercontent.com/evgenyb/azfw-perf/refs/heads/main/iac/scripts/server.sh'
        ]
        commandToExecute: 'sh server.sh'
      }
    }
    osType: 'Linux'
    vmSize: vmSize
...
output serverVNetId string = modVNet.outputs.resourceId
